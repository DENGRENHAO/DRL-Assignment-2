...
class NTupleNetwork:
    ...
    def save_weights(self, filename):
        ...
        with open(f"models_new/{filename}", "wb") as f:
            pickle.dump(serializable_weights, f)

    def load_weights(self, filename):
        """Load the network weights from a file"""
        with open(f"models_new/{filename}", "rb") as f:
...
class ThreeStageTDLearning:
    def __init__(self, alpha=0.1, gamma=1.0, exploration=0.1, window_size=1000, 
        ...
        self.t_16k = 12  # First 4096-tile (2^12)
        self.t_16k_8k = (12, 11)  # Both 4096 (2^12) and 2048 (2^11) tiles
        ...
        self.stage_updates = [0, 0, 0]  # Count of updates for each stage
        self.stage_scores = [[], [], []]  # Scores for games where each stage was reached
    ...
    def learn_from_trajectory(self, trajectory):
        ...
        for i in range(len(trajectory) - 1, -1, -1):
            ...
            if done:
                ...
                # Use the stage of the next state for evaluation
                target = next_reward + self.gamma * self.network.evaluate(next_after_state.copy(), stage=next_stage)
                
                # Update the value function for the after_state using the current stage's weights
                self.network.update(after_state.copy(), target, stage=stage, alpha=self.alpha)
                self.stage_updates[stage] += 1
    
    def train(self, num_episodes=5000000, start_episode=0, weights_file=None):
        """Train all stages in a single pass"""
        if start_episode > 0:
            print(f"Resuming training from episode {start_episode}...")
            # Load weights from the specified episode
            self.network.load_weights(f"weights_ep{start_episode}.pkl")
        elif weights_file:
            print(f"Loading weights from {weights_file}...")
            # Load weights from the specified file
        ...
        for episode in progress_bar:
            ...
            # Track max stage reached in this episode
            episode_max_stage = 0
            ...
            while not done:
                # Select and perform action using the stage of the current board
            ...
            # Record score for the highest stage reached
            self.stage_scores[episode_max_stage].append(self.env.score)
            
            # Update max reached stage for display
            max_reached_stage = max(max_reached_stage, episode_max_stage)
            
            # Check for progress (every window_size episodes)
            if (episode + 1) % self.window_size == 0:
                ...
                stage_stats = []
                for stage in range(3):
                    if self.stage_updates[stage] > 0:
                        stage_stats.append(f"S{stage+1}: {self.stage_updates[stage]}")
                
                progress_bar.set_description(
                    f"Ep {episode+1}, Score: {avg_score:.0f}, Max: {max_tile}, " +
                    f"Stages: {'/'.join(stage_stats)}, Max Stage: {max_reached_stage+1}"
                )
                ...
                if (episode + 1) % self.weight_saving_interval == 0:
                    self.network.save_weights(f"weights_ep{episode+1}.pkl")
        
        # Save final weights
        self.network.save_weights("weights_final.pkl")
        
        print("Training complete.")
        print(f"Stage updates: Stage 1: {self.stage_updates[0]}, Stage 2: {self.stage_updates[1]}, Stage 3: {self.stage_updates[2]}")
    ...
    def plot_learning_curves(self):
        """Plot learning curves for all stages"""
        plt.figure(figsize=(12, 8))
        
        # Plot overall score progress
        avg_scores = []
        ...
        plt.subplot(2, 2, 1)
        plt.plot(range(len(avg_scores)), avg_scores, label='Overall Average Score', color='blue')
        plt.title('Overall Learning Curve')
        plt.xlabel(f'Episodes (x{self.window_size})')
        plt.ylabel('Average Score')
        plt.grid(True, alpha=0.3)
        
        # Plot stage-specific scores if available
        plt.subplot(2, 2, 2)
        for stage in range(3):
            if len(self.stage_scores[stage]) > 0:
                window_avg = []
                for i in range(0, len(self.stage_scores[stage]), max(1, min(self.window_size, len(self.stage_scores[stage]))//10)):
                    window = self.stage_scores[stage][i:i+max(1, min(self.window_size, len(self.stage_scores[stage]))//10)]
                    if window:
                        window_avg.append(np.mean(window))
                
                if window_avg:
                    plt.plot(window_avg, label=f'Stage {stage+1}', marker='o', markersize=3)
        
        plt.title('Stage-Specific Score Progress')
        plt.xlabel('Updates Window')
        plt.ylabel('Average Score')
        plt.legend()
        plt.grid(True, alpha=0.3)
        
        # Plot update counts for each stage
        plt.subplot(2, 2, 3)
        stages = ['Stage 1', 'Stage 2', 'Stage 3']
        plt.bar(stages, self.stage_updates, color=['blue', 'green', 'red'])
        plt.title('Weight Updates per Stage')
        ...
        plt.subplot(2, 2, 4)
        max_tile_counts = {}
        for tile in self.max_tiles_history:
            max_tile_counts[tile] = max_tile_counts.get(tile, 0) + 1
        ...
        plt.tight_layout()
        plt.savefig(f'models_new/learning_curves.png')
        plt.close()

# Create the trainer with unified training approach
trainer = ThreeStageTDLearning(
    alpha=0.1, 
    gamma=1.0, 
    exploration=0.1, 
    window_size=10, 
    weight_saving_interval=2500, 
    search_depth=0, 
    vinit=0
)

# Train with the multi-stage approach
trainer.train(num_episodes=100000, start_episode=0, weights_file="stage1_weights_ep35000_new.pkl")

# # Evaluate the agent
# trainer.search_depth = 2  # Set search depth for evaluation
# scores, max_tiles = trainer.evaluate_agent(num_games=20)